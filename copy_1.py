...
def find_top_n_similar_audios(target_file, top_n, progress_bar, progress_label, stop_event):
    ...
    similarities = []
    with ProcessPoolExecutor(max_workers=10) as executor:  # 控制最大并发任务数为10
...
class AudioSimilarityApp(tk.Tk):
    ...
    def find_similar_audios(self):
        ...
        if feature_manager_instance.feature_file is None:
            ...
            if not feature_file:
                messagebox.showwarning("Input Error", "Please select a feature file.")
                return
            feature_manager_instance.set_feature_file(feature_file)

        self.stop_event.clear()
        self.progress_bar['value'] = 0
        self.progress_label.config(text="Starting similarity search...")
        self.update()

        # 使用线程来执行相似音频查找任务
        threading.Thread(target=self.run_similarity_search, args=(target_file,)).start()

    def run_similarity_search(self, target_file):
        top_n = 10  # Number of top similar audios to retrieve
        results = find_top_n_similar_audios(target_file, top_n, self.progress_bar, self.progress_label, self.stop_event)
        
        self.listbox_result.delete(0, tk.END)
        for file_path, similarity in results:
            self.listbox_result.insert(tk.END, f"{os.path.basename(file_path)} - Similarity: {similarity:.2f}")
    ...
    def set_feature_file(self):
        ...
        if feature_file:
            feature_manager_instance.set_feature_file(feature_file)
            self.label_feature_file.config(text=f"Current Feature File: {os.path.basename(feature_file)}")
    ...
    def play_audio(self, event):
        ...
        file_path = os.path.join(self.entry_dir.get(), file_name)
