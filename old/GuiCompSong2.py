...
import subprocess
import platform
# 主程序中使用 feature_manager_instance
from feature_manager_gai import feature_manager_instance  # 重命名实例避免冲突
...
class AudioSimilarityApp(tk.Tk):
    ...
    def find_similar_audios(self):
        ...
        # 使用线程来执行音频比较任务
        threading.Thread(target=self.run_find_similar, args=(target_file, top_n)).start()

    def run_find_similar(self, target_file, top_n):
        top_n_similar_files = find_top_n_similar_audios(target_file, top_n, self.progress_bar, self.progress_label, self.stop_event)
        self.run_find_similar_continue(top_n_similar_files)
